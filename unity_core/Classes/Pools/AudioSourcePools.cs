...
public class AudioSourcePools : MonoBehaviour
{
    ...
    void Awake()
    {
        if (instance != null) Log.Exception("实例已经存在:AudioSourcePools");
        instance = this;
    ...
    /// <returns></returns>
    public AudioSource SpawnByFile(string file, Vector3 pos, Transform parent)
    {
    ...
    /// 产生声音
    /// </summary>
    public AudioSource SpawnByClip(AudioClip clip, Vector3 pos, Transform parent = null)
    {
    ...
    public void Clear()
    {
        foreach (var list in m_DicFile2AudioPool)
        {
            foreach (var obj in list.Value)
            {
                if (obj != null && obj.gameObject != null) GameObject.Destroy(obj.gameObject);
            }
        }
